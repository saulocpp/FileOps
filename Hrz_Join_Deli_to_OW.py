# Horizon names are limited to 60 characters as per the format file, so file names will be internally trimmed at the end
# Use file format HorizonExportv1p0_3d.afm.xml in Data Import to load
...
def hrz_deli2ow(file_list, interp, survey, domain, onset):
	with open("Deli2OW_" + datetime.now().strftime("%d-%m-%Y_%H%M%S") + ".dat", "w") as d2ow:
		for file_name in file_list:
			...
			ATTRIBUTE		= domain + "_STRUCTURE" + " " * (25 if len(domain) == 5 else 26)
			...
			for i in range(1, LINE_COUNT):
				if(str(deli["f2"][i]) != "nan" and str(deli["f3"][i]) != "nan"):
